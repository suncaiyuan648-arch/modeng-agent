...
| `ARCH006` | `PUBLIC_CONTRACT_UNAUTHORIZED_CHANGE` | A frozen Contract changed without an approved CCR present in `BASE_SHA`.        |
...
| `ARCH011` | `ARCHITECTURE_REVIEW_REQUIRED`        | A protected path is outside approved `BASE_SHA` scope or lacks required review. |
